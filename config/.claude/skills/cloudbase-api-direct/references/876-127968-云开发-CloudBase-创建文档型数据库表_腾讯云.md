...
[点击调试](https://console.cloud.tencent.com/api/explorer?Product=tcb&Version=2018-06-08&Action=CreateTable)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/876/34812) ，本接口取值：CreateTable。 |
...
| TableName | 是 | String | 数据表名；长度不超过96个字符，可以为英文字母、数字、下划线(\_)和短横线(-)的组合，且不能以下划线开头  
示例值：in |
| Tag | 否 | String | FlexDB实例ID，如：tnt-nl7hjzasw  
示例值：tnt-nl7hjzasw |
| PermissionInfo | 否 | [PermissionInfo](/document/api/876/34822#PermissionInfo) | FlexDB数据库权限信息 |
...
```
https://tcb.tencentcloudapi.com/?Action=CreateTable
&TableName=adsia
&Tag=tnt-nl7hjzasw
&<公共请求参数>
```
...
| FailedOperation.CreateTable | FailedOperation.CreateTable |
...
| LimitExceeded.OutOfTableQuota | LimitExceeded.OutOfTableQuota |
...
| ResourceUnavailable.ResourceExist | ResourceUnavailable.ResourceExist |
