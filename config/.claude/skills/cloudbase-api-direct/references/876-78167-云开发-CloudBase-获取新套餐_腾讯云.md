...
[点击调试](https://console.cloud.tencent.com/api/explorer?Product=tcb&Version=2018-06-08&Action=DescribeBaasPackageList)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/876/34812) ，本接口取值：DescribeBaasPackageList。 |
| Version | 是 | String | [公共参数](/document/api/876/34812) ，本接口取值：2018-06-08。 |
| Region | 否 | String | [公共参数](/document/api/876/34812) ，本接口不需要传递此参数。 |
| PackageName | 否 | String | tcb产品套餐ID，不填拉取全量package信息。  
示例值：baas\_personal |
| EnvId | 否 | String | 环境ID  
示例值：env-xxyyzzaabb |
| Source | 否 | String | 套餐归属方，填写后只返回对应的套餐 包含miniapp与qcloud两种 默认为miniapp  
示例值：miniapp |
| EnvChannel | 否 | String | 套餐归属环境渠道  
示例值：qc\_console |
| TargetAction | 否 | String | 拉取套餐用途：  
1）new 新购  
2）modify变配  
3）renew续费  
示例值：new |
| GroupName | 否 | String | 预留字段，同一商品会对应多个类型套餐，对指标有不同侧重。  
计算型calculation  
流量型flux  
容量型capactiy  
示例值：calculation |
| PackageTypeList.N | 否 | Array of String | 类型分组过滤。默认为\["default"\]  
示例值：\["default","vip"\] |
| PaymentChannel | 否 | String | 付费渠道，与回包billTags中的计费参数相关，不填返回默认值。  
示例值：miniapp |
...
| 参数名称 | 类型 | 描述 |
| --- | --- | --- |
| PackageList | Array of [BaasPackageInfo](/document/api/876/34822#BaasPackageInfo) | 套餐列表 |
