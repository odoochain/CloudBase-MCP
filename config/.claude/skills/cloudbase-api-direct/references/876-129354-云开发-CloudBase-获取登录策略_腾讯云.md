...
[点击调试](https://console.cloud.tencent.com/api/explorer?Product=tcb&Version=2018-06-08&Action=DescribeLoginConfig)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/876/34812) ，本接口取值：DescribeLoginConfig。 |
...
| 参数名称 | 类型 | 描述 |
| --- | --- | --- |
| EmailLogin | Boolean | 是否开启邮箱登录方式。true 表示已开启，允许用户使用邮箱和密码进行登录；false 表示已关闭。  
示例值：true |
| AnonymousLogin | Boolean | 是否开启匿名登录方式。true 表示已开启，允许用户无需注册即可以匿名身份登录；false 表示已关闭。  
示例值：true |
| UserNameLogin | Boolean | 是否开启用户名密码登录方式。true 表示已开启，允许用户使用用户名和密码进行登录；false 表示已关闭。  
示例值：true |
| SmsVerificationConfig | [VerificationConfig](/document/api/876/34822#VerificationConfig) | 短信验证码发送配置，包含短信发送通道类型、自定义 APIs 数据源、调用方法及每日发送限额等信息。 |
| PhoneNumberLogin | Boolean | 是否开启手机号短信登录方式。true 表示已开启，允许用户使用手机号和短信验证码进行登录和注册；false 表示已关闭。  
示例值：true |
| MfaConfig | [MFALoginConfig](/document/api/876/34822#MFALoginConfig) | MFA 多因子认证登录配置，包含 MFA 开关及各验证方式（短信、邮箱、TOTP、强制绑定手机号）的启用状态。  
注意：此字段可能返回 null，表示取不到有效值。 |
| PwdUpdateStrategy | [PasswordUpdateLoginConfig](/document/api/876/34822#PasswordUpdateLoginConfig) | 密码修改策略配置，包含首次登录强制修改密码开关及定期修改密码策略（周期和时间单位）。  
注意：此字段可能返回 null，表示取不到有效值。 |
...
X-TC-Action: DescribeLoginConfig
<公共请求参数>
