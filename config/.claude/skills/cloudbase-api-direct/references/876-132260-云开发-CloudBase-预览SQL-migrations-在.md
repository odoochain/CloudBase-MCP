...
## 预览SQL migrations 在远端的执行计划，不实际执行 SQL
...
[点击调试](https://console.cloud.tencent.com/api/explorer?Product=tcb&Version=2018-06-08&Action=PreviewPGUserMigrations)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/876/34812) ，本接口取值：PreviewPGUserMigrations。 |
...
| Migrations.N | 是 | Array of [MigrationInput](/document/api/876/34822#MigrationInput) | 
...
| 参数名称 | 类型 | 描述 |
| --- | --- | --- |
| Pending | Array of [MigrationPlanItem](/document/api/876/34822#MigrationPlanItem) | 
将要执行的migration列表

 |
| Applied | Array of [MigrationPlanItem](/document/api/876/34822#MigrationPlanItem) | 

已经应用的migration列表

 |
| Conflicts | Array of [MigrationConflict](/document/api/876/34822#MigrationConflict) | 
...
X-TC-Action: PreviewPGUserMigrations
<公共请求参数>
    ...
    "Migrations": [
        {
            ...
            "Rollback": "DROP TABLE IF EXISTS public.test;"
        }
    ],
    "Source": "cli"
}
